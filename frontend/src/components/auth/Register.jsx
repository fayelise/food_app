...
const Register = () => {
  ...
  return (
    <div className="w-full h-screen flex items-center justify-center">
      ...
      <Link to="/"><img src={logo} alt="bible" className="w-20 md:w-28" /></Link>
      <h1 className="text-lg md:text-2xl font-semibold text-gray-700">Welcome to your Big-Back App</h1>
      {message.message && (<Message message={message.message} type={message.type} clearMessage={() => setMessage({ message: "", type: "" })}/>)}
      <p className="text-md md:text-lg text-center text-gray-700">Already have an account? <Link to="/login" className="text-blue-500">Sign In</Link></p>
      <div className="w-full  flex flex-col gap-3">
        ...
        <div className="w-full flex items-center bg-white border-[#FF1212] border p-3 rounded-xl gap-2 shadow-lg relative">
      ...
      <button className="w-full bg-[#FF1212] text-white rounded-xl mt-3 py-3 text-sm md:text-base" onClick={handleRegister}>Register</button>
      </div>
    </div>
  )
}
